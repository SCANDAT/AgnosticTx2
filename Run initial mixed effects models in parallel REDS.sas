...
dm 'odsresults; clear';
dm 'log; clear';
filename output3 "&localpath\SAS Outputs\Run initial mixed effects models in parallel REDS, &tw hours.pdf";
ods pdf file=output3;

*3. Fetch data;
proc sort data=agtx.finaldata_reds_long&tw;
by label predictor;
run;
/*
proc means data=agtx.finaldata_reds_long&tw min p1 q1 median mean q3 p99;
var deltavalue;
where label="EVF";
run;
...
proc summary data=agtx.finaldata_reds_long&tw;
where label="HB" ;
by label predictor;
...
if predictor="numdoncat" then do;
	do predictorvalue=0,1,5,10,15,20;
	    output;
	    end;
	end; 
...
*5. Count runs;
proc sql;
select count(*) into: noruns from (select distinct label, predictor from agtx.finaldata_reds_long&tw);
quit;
...
%inc "&localpath\Run as serial mixed models using SAS HPMIXED REDS parallelized.sas";
...
data agtxoutp.univariate_reds&tw._plotdata;
set plotdata_1-plotdata_&nosessions;
drop bloodgroup--donationtype;
run;
data agtxoutp.univariate_reds&tw._modelfit;
set convergence_1-convergence_&nosessions;
run;
data agtxoutp.univariate_reds&tw._parmests;
set fixedeffects_1-fixedeffects_&nosessions;
run;
data agtxoutp.univariate_reds&tw._type3;
set type3tests_1-type3tests_&nosessions;
run;

*Summarize fit results;
proc freq data=agtxoutp.Univariate_reds&tw._modelfit;
...
proc sort data=agtxoutp.univariate_reds&tw._type3 out=crude_pvals;
where effect =: 'pred';
by adjusted probf;
...
proc sql;
create table agtxoutp.univariate_reds&tw._fdr(drop=idnr) as
select
  b.*,
  a.fdr_p
...
data _null_;
file "K:\SCANDAT\User\Gustaf\AgTxFigure1\src\data\plotdata_reds&tw..csv" delimiter=',' DSD DROPOVER lrecl=32767;
...
run;
data _null_;
file "K:\SCANDAT\User\Gustaf\AgTxFigure1\src\data\fdr_reds&tw..csv" delimiter=',' DSD DROPOVER lrecl=32767;
if _n_ = 1 then        /* write column names or labels */
do;
 put
    "predictor"
...
run;
dm 'log; print file="&localpath\Logs\Run initial mixed effects models in parallel REDS, &tw hours.log" replace; ';
ods pdf close;
ods html;
